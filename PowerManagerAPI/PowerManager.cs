...
public static class PowerManager
{
    ...
    public static void SetPlanName(Guid planId, string name)
    {
        ...
        var bufferSize = (uint)Encoding.Unicode.GetByteCount(name);
    ...
    public static void SetPlanDescription(Guid planId, string description)
    {
        ...
        var bufferSize = (uint)Encoding.Unicode.GetByteCount(description);
    ...
    /// <param name="powerSource">Power mode. AC or DC, but not both.</param>
    /// <returns>The active index value for the specified setting</returns>
    public static uint GetPlanSetting(Guid plan, SettingSubgroup subgroup, Setting setting, PowerSource powerSource)
    {
        if (powerSource == (PowerSource.AC | PowerSource.DC))
    ...
    ///     You can chose to alter the AC value, the DC value or both using the bitwise OR operator (|)
    ///     to join the flags.
    /// </param>
    /// <param name="value">
    ///     The new value for the setting. Run <code>powercfg -q</code> from the command line to list possible
    ///     values
    /// </param>
    public static void SetPlanSetting(Guid plan, SettingSubgroup subgroup, Setting setting, PowerSource powerSource, uint value)
    {
        var subgroupId = SettingIdLookup.SettingSubgroupGuids[subgroup];
        var settingId = SettingIdLookup.SettingGuids[setting];
        ...
        if (powerSource.HasFlag(PowerSource.DC))
        {
    ...
    /// <summary>
    ///     Creates a list of all the power plan Guids on this PC. The Guids can be used to look up more information (name,
    ///     settings, etc.) about each plan.
    /// </summary>
    ...
    public static PowerPlan ActivePowerPlan
    {
        get
        {
            var result = GetActivePlan();

            if (result == PowerPlanGuids.Balanced)
                return PowerPlan.Balanced;
            if (result == PowerPlanGuids.HighPerformance)
                return PowerPlan.HighPerformance;
            if (result == PowerPlanGuids.PowerSaver)
                return PowerPlan.PowerSaver;
            if (result == PowerPlanGuids.UltimatePerformance)
        ...
        set
        {
            SetActivePlan(
                value switch
                {
                    PowerPlan.Balanced => PowerPlanGuids.Balanced,
                    PowerPlan.HighPerformance => PowerPlanGuids.HighPerformance,
                    PowerPlan.PowerSaver => PowerPlanGuids.PowerSaver,
                    PowerPlan.UltimatePerformance => PowerPlanGuids.UltimatePerformance,
    ...
    public static PowerMode PowerMode
    {
        get
        {
            ...
            if (actualOverlayGuid == PowerModeGuids.BestPowerEfficiency)
                return PowerMode.BestPowerEfficiency;
            if (actualOverlayGuid == PowerModeGuids.Balanced)
                return PowerMode.Balanced;
            if (actualOverlayGuid == PowerModeGuids.BestPerformance)
        ...
        set
        {
            PowerSetActiveOverlayScheme(
                value switch
                {
                    PowerMode.BestPowerEfficiency => PowerModeGuids.BestPowerEfficiency,
                    PowerMode.Balanced => PowerModeGuids.Balanced,
                    PowerMode.BestPerformance => PowerModeGuids.BestPerformance,
                    _ => throw new ArgumentOutOfRangeException(nameof(value), value, null),
                });
        }
    }
    ...
    [DllImport("powrprof.dll")]
    private static extern uint PowerGetActualOverlayScheme(
